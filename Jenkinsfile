pipeline{
    ...
    stages{
        ...
        stage('Replace domain names') {
            steps {
              script {
                sh 'cp -r ./kube/initdb /home/jenkins/initdb'
       ...
       stage('Docker build all images'){
        steps{
            script{
                ...
                sh 'docker build -t lanico/natural-presence-db:latest directus/ '
            }
        }
       }
       ...
       stage('Deploy to kubernetes'){
        steps{
            ...
            step([$class: 'KubernetesEngineBuilder', projectId: env.PROJECT_ID, clusterName: env.CLUSTER_NAME, location: env.LOCATION, manifestPattern: 'kube/db.yml', credentialsId: env.CREDENTIALS_ID, verifyDeployments: false])
